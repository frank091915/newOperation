...
export default {
  ...
  methods: {
    ...
    GetposMechineRoomStatements(page,timeScale,isSearching){
      this.isLoading=true;
    ...
    seeDetails(item){
        ...
        this.$nextTick(()=>{
            this.$router.push({ path: "/posMechineRoomStatementsDetails", query: { title: printTitle,id:item.id,startTime:item.startTime.substring(0,10),type:item.type,menuIconId:6}});
        })
  ...
  created() {
    ...
    // 默认请求第一页，按周统计
    this.GetposMechineRoomStatements(1,2,false)
