...
export default {
  ...
  methods: {
    print(){
window.open(`${JSON.parse(window.sessionStorage.getItem("url")).baseURL}/api/export/server/detail?type=${this.$route.query.type}&startTime=${this.$route.query.startTime}&accessToken=${JSON.parse(window.sessionStorage.getItem("operationToken"))}`,"blank")
    },
    changeTimeScale(e){
    ...
    //   });
    // },f  
    GetServerStatementsDetails(){
        this.isLoading=true;
        this.$http.toGetServerStatementsDetails(this.$route.query.type,this.$route.query.startTime).then((res)=>{
            // console.log(res)
            if(res.data.success){
                this.data=res.data.data.map((item)=>{
  ...
  created() {
    ...
    this.GetServerStatementsDetails()
