...
export default {
  ...
  methods: {
    ...
    search(isSearching) {
        this.$nextTick(()=>{
          this.GetNetworkStatements(this.page,this.timeScale,isSearching)
        })
    ...
    GetNetworkStatements(page,timeScale,isSearching){
      this.isLoading=true;
      this.$http.toGetNetworkStatements(page,timeScale).then(res => {
        if (res.data.success) {
    ...
    seeDetails(item){
        console.log(item)
        this.$router.push({ path: "/networkStatementsDetails", query: { title: this.printTitle,id:item.id,startTime:item.startTime.substring(0,10),type:item.type}});
    }
  },
  computed:{
    printTitle (){
  ...
  created() {
    ...
    // 默认请求第一页，按周统计
    this.GetNetworkStatements(1,2,false)
