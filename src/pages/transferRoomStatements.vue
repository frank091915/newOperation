...
export default {
  ...
  methods: {
    ...
    seeDetails(item){
        ...
        this.$nextTick(()=>{
            this.$router.push({ path: "/transferRoomStatementsDetails", query: { title: printTitle,id:item.id,startTime:item.startTime.substring(0,10),type:item.type,menuIconId:6}});
        })
    ...
    Title (){
      ...
      switch(this.timeScale){
        ...
        return '圈存机房间周次巡检表';
        case 3:
        return '圈存机房间'+ month+'月次巡检表';
        case 4:
        return '圈存机房间'+ year+'年次巡检表';
      }
    }
  },
  computed:{
